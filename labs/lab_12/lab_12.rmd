...
**A1. The leading word is failing, and I think that the top 10 words describe a fallout between a client and their lawyer. More specifically, I think that this has to do wtih a lawyer going through his/her clients' account with the client's consent.**  

```{r}
md_attorney_sanctions <- read.csv("data/md_attorney_sanctions.csv")
...
```{r}
data("stop_words")
finished_unique_words <- unique_words |> anti_join(stop_words)
```
```{r}
finished_unique_words |>
  group_by(word) |>
  tally(sort=TRUE) |>
