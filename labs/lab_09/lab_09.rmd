...
Chat GPT: 
```{r}
md_votes_by_types <- read_csv("data/md_votes_by_type.csv") |>
clean_names()
```
...
md_votes_by_type
  group_by(year) |>
  summarize(
    total_votes = sum(early, election_day, mail, na.rm = TRUE),
    early = sum(early, na.rm = TRUE),
    election_day = sum(election_day, na.rm = TRUE),
    mail = sum(mail, na.rm = TRUE),
    .groups = 'drop'  # Drop grouping after summarizing
  ) |>
  mutate(
    early_Percentage = (early / total_votes) * 100,
    election_day_percentage = (election_day / total_votes) * 100,
    mail_percentage = (mail / total_votes) * 100
  ) |>
  select(year, early_Percentage, election_day_percentage, mail_percentage)
