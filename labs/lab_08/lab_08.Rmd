...
```{r}
md_absentee_ballots |>
  mutate(percent_return = `TOTAL RECEIVED`/ `TOTAL SENT`*100) |>
  arrange(desc(percent_return)) |>
  slice(1:10) |>
  ggplot(aes(x = reorder(`COUNTY NAME`, percent_return), y = percent_return)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(
    title = "Highest Percentage of Absentee Ballots Returned", 
    y = "Percentage Returned",
    x = "County",
...
```{r}
md_absentee_ballots |>
  mutate(democrat_percent_return = `DEM RECEIVED`/DEM_SENT *100) |>
  arrange(democrat_percent_return) |>
  slice(1:10) |>
  ggplot(aes(x = reorder(`COUNTY NAME`, democrat_percent_return), y = democrat_percent_return)) + 
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(
    title = "Lowest Percentage of Ballots Returned by Democratic Voters",
    y = "County", 
    x = "Percent Returned",
    caption = "Source: Maryland State Board of Elections"
  )
...
party_difference <- md_absentee_ballots |>
  mutate(
    total_votes = `DEM RECEIVED` + `REP RECEIVED`,
    dem_percentage = (DEM RECEIVED / total_votes) * 100,
    rep_percentage = (REP RECEIVED / total_votes) * 100,
    percentage_diff = (DEM RECEIVED - REP RECEIVED))
```
...
md_absentee_dates |>
  ...
  geom_line(aes(x = date, y = `OTH RECEIVED`), color = "black") +
  labs(
    title = "Returned Ballots for Frederick County for All parties",
    x = "Return Date",
    y = "Ballots Recieved",
    caption = "Source: Maryland State Board of Elections"
  )
