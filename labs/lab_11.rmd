...
```{r}
FC_url <- "https://elections.maryland.gov/elections/2024/general_Results/gen_results_2024_by_county_11.html"

FC_url <- read_html(FC_url) |>
  html_table()

Boe_results <- FC_url [(9)]

Boe_results |>
  mutate( early_voting = as.numeric(str_remove(early_voting, ",")),
    election_day = as.numeric(str_remove(election_day, ",")),
    mail_in_ballot = as.numeric(str_remove(provisional, ",")),
  ...
  labs(y = "percentage",
       x = "candidates",
       title = "Board of Education Results")
```
...
Results <- 'https://osp.maryland.gov/category/press-releases/' |>
  read_html () |>
  html_element("article a")

tibble(
  title = FC_url |> html_text(trim = TRUE),
  url = FC_url |> html_attr("href")
) |>
