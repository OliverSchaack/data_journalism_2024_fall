...
* **Our analysis shows that in general, early voting has increased across counties in Maryland, with some like Washington and Calvert counties more than tripling their early voting turnout over the 10-year stretch. One thing that is a little surprising is the sharp decline in EV in Baltimore City, which began before the pandemic and seems to only now be slightly recovering. The only other county jurisdictions with declines from 2014-2024 are Somerset, Dorchester, Kent, and Anne Arundel, which do not have much in common with Baltimore City. The gubernatorial races tend to draw less early voting, which is not surprising because they draw less voting overall. Three counties did not have a single early voter in the 2022 race.**:
...
md_ev_combined <- md_ev_pres_24 |>
  ...
  left_join(md_ev_gub_14, join_by(county_name)) |> #join all the dataframes at once because they all have the same structure
  mutate(pct_change=100*(total_24-total_14)/total_14) |> #new column to show % change from 2014 to 2024
...
md_ev_pres_16 <- clean_names(read_csv("data/md_ev_pres_16.csv")) |>
  group_by(party_code)|>
  summarise(count = n()) #counting EV by party for 2016

md_ev_pres_20 <- clean_names(read_csv("data/md_ev_pres_20.csv")) |>
  group_by(party_code)|>
  summarise(count = n()) #counting EV by party for 2020

md_ev_pres_24 <- clean_names(read_csv("data/md_ev_pres_24.csv")) |>
  group_by(party_code)|>
  summarise(count = n()) #counting EV by party for 2024
...
* **Early voting data was pervasive in Pennsylvania and Delaware, so we looked at Virginia as our neighborly comparison. As we can see from our analysis, a higher percentage of VA residents participated in early voting in 2024. Since Virginia's population is also higher than Maryland's, this means that almost 150,000 more people early voted in VA than in MD.
**:
...
va_ev_pres_24 <- clean_names(read_csv("data/va_ev_pres_24.csv")) |>
  select(early_voting) |>
  summarise(
