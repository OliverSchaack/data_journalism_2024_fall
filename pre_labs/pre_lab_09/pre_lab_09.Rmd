...
**https://www.datawrapper.de/_/SwNeq/** Copy the url of the graphic you published here. It will begin with <https://datawrapper.dwcdn.net/>. Don't copy the browser's current url.
...
**Task** Look for "Hide regions without data" under Appearance, and click the slider icon to enable that feature. You should see a map zoomed into Maryland with some counties in various colors. **Answer** It added color to the state of Maryland based on counties with different voting percentages.
...
**Task** Click the "Customize tooltips" button so it expands own. Change {{ fips_code }} to {{ county }} and {{ pctmoore }} to {{ FORMAT(pctmoore, "00.0%")}} **It says there's an error** Describe the changes from the previous map
...
**Task** Click the "Proceed" button until you get to the "Publish & Embed" tab, then click "Publish Now". Copy the published URL (like you did for the chart above) and paste it below. **https://datawrapper.dwcdn.net/u89p4/1/** Put the URL of your map here.
