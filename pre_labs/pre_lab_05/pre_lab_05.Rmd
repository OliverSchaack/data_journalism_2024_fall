...
**Task** Load the five CSV files. Do they have the same column structure? **They have the same physical strucutre, however the order of the columns varies from file to file.**
...
Answer - It combines all the data from each county from 2016 to 2024 when voting took place. The structure is stil the same just with more data now.
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **There are now 24 rows, which is how many there should be because there were 24 rows prior to this code being ran.**
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **It says that it is joining with " 'by = join_by(COUNTY)"**
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **The county with the highest 18+ black population as a percentage of all population 18 and over is Prince George's County. The percentage of democracts in PG County is just over 78%.**
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **The county with the highest 18+ Asian population as a percentage is Howard County and the percent of democrats in that county in 52%. There's a large difference in highest percentage county of black 18+ and asian 18+. However, what is interesting is that even with this discrepancy, still over half the Asian population is decmocratic which if I were to infer off these data sets makes me believe that there is a high democractic community in Maryland.**
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **It is dbl data.**
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **It is now <chr> data**
